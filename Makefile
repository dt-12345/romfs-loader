...
PROGRAM_ID_STRING := $(shell echo $(PROGRAM_ID) | tr '[:lower:]' '[:upper:]')
...
EXL_CFLAGS   := $(C_FLAGS) -DEXL_LOAD_KIND=$(LOAD_KIND) -DEXL_LOAD_KIND_ENUM=$(LOAD_KIND_ENUM) -DEXL_PROGRAM_ID_HEX=0x$(PROGRAM_ID) -DEXL_PROGRAM_ID=$(PROGRAM_ID_STRING)
